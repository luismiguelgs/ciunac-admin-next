...
export default function OptSubjects() 
{
    const loadData = async () => {
        const data = await OpcionesService.fetchItems<IBaseData>(Collection.Cursos)
        setRows(data)
    }
    //Hooks *****************************************************************
    ...
    const handleConfirmDelete = async () => {
        if (idToDelete) {
            await OpcionesService.deleteItem(Collection.Cursos, idToDelete as string);
    ...
    const processRowUpdate = async(newRow: GridRowModel) => {
        ...
        if(newRow.isNew){
            id = await OpcionesService.newItem(Collection.Cursos, newRow as IBaseData)
        }else{
            OpcionesService.updateItem(Collection.Cursos, newRow as IBaseData)
        }
